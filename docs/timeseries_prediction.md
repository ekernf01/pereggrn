...
We previously assumed one prediction for every test-set sample. We no longer enjoy this luxury, and instead, we must deliberately select fair pairings of predicted and observed samples. Specifically: 
...
- For some methods, the simulation is moving forward in time. For most methods, the simulation is really just describing cell-state-specific perturbation effects with minimal to no progress in time. We need to make sure each type of method has a chance to "reach" the final time-point: for PRESCIENT, this means selecting the right trajectory length according to the starting state, and for local-effect-only methods, this means starting with the final timepoint.
- The plan above hits one pitfall when KO's cause a block in differentiation, as in the endoderm data. So, we need to choose a starting cell type that gives each method a fighting chance to predict these extreme outcomes. In these cases, it would be more sensible to relax the requirement that `timepoint` in the prediction metadata be equal to `timepoint` in the observed expression data. In practice, users of e.g. CellOracle would use **all(()) starting states. They would look for a strong effect inhibiting differentiation at an intermediate timepoint, which could be selected as the most interesting hypothesis and compared directly to the observed data.
- Long-term simulation creates problems for cell type labeling. In complex timeseries such as the Saunders whole zebrafish embryo scRNA, it is hard to name the precursor that will give rise to a specified terminal cell type 48 or 72 hours later. In other words, there is a difficult lineage tracing problem embedded inside the causal GRN benchmarking problem. GGRN and PRESCIENT offer various solutions (see discussion of `matching_method` in the [GGRN exposition](https://github.com/ekernf01/ggrn/blob/main/docs/GGRN.md)), but these are approximations for purposes of training specific models. For ground truth, we need an approach that is neutral and data-driven. The current plan is to assign each prediction a cell type label using a classifier, then compare each prediction against the cell type that the classifier predicts. We will also simplify the Saunders data, feeding in smaller lineage trees instead of the whole embryo.
...
- Compare predictions to test data within each cell type and timepoint, averaging together all test samples. This will require new code inside of `evaluator.evaluateCausalModel` to do the averaging and it will require a classifier to relabel the cell type according to the predicted expression. Especially for PRESCIENT, we need to allow the predicted expression to match a different cell type compared to the starting cell type. 
...
- Zebrafish: We can use the original authors' cell type annotation. 
- Endoderm: this is a tricky case because the knockdowns yield one cell type that doesn't occur in the timeseries data and two others that don't match exactly. See [fig. 2c](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6525305/). Currently we use "PSC", "primitive streak", "endoderm", or "ectopic". This is an accurate summary of the biology, but it raises a problem: methods using cell type specific networks will not have an "ectopic" network available because that cell type is by definition absent from the training data. This is a brutally hard test case, so it should be low priority. For now, let's substitute in all cell types from the training data, get the predictions, but leave the evaluation code as it is. We will probably want to do manual evaluation of this difficult case.
